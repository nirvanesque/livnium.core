...
class SemanticWarp:
    ...
    def find_warp_path(
        ...
        TENSION-PRESERVING: Penalizes diagonal moves that misalign ontology
        and long jumps to preserve contradiction signals.
        ...
        for i in range(m):
            for j in range(n):
                ...
                if i > 0:
                    # Move from above (skip premise word)
                    # Penalize long jumps: add penalty for skipping multiple positions
                    skip_penalty = 0.1  # Small penalty for skipping
                    energy = dp[i-1, j] + distance_matrix[i, j] + skip_penalty
                    candidates.append((energy, (i-1, j)))
                
                if j > 0:
                    # Move from left (skip hypothesis word)
                    skip_penalty = 0.1  # Small penalty for skipping
                    energy = dp[i, j-1] + distance_matrix[i, j] + skip_penalty
                    candidates.append((energy, (i, j-1)))
                
                if i > 0 and j > 0:
                    # Move diagonally (one-to-one alignment)
                    # TENSION-PRESERVING: Penalize diagonal moves that misalign ontology
                    # If distance is high, diagonal alignment is forced but penalized
                    diagonal_penalty = 0.0
                    if distance_matrix[i, j] > 0.7:  # High distance = misalignment
                        # Penalize forcing alignment of mismatched words
                        diagonal_penalty = distance_matrix[i, j] * 0.3
                    
                    energy = dp[i-1, j-1] + distance_matrix[i, j] + diagonal_penalty
