...
- `text/` – Encoders: `encoder.py` (legacy embedding mean-pool, unused for SNLI), `quantum_text_encoder.py` bridge to `../quantum_embed`, `ecw_bt_encoder.py` for ECW-BT pairwise/rotated embeddings. Legacy `geom_encoder.py`/`sanskrit_encoder.py` have been removed.
- `tasks/snli/` – SNLI-specific glue: `encoding_snli.py` builds the initial state by sequentially adding premise + hypothesis (v_p + v_h) with the quantum encoder; `head_snli.py` classification head with explicit geometry features.
- `training/train_snli_vector.py` – End-to-end training loop with dataset loading, optional dynamic basins, and the quantum encoder.
...
- **Encoders**: Build OM/LO vectors and initial state `h0 = v_p + v_h` (with small noise for symmetry breaking).
  - `quantum`: Pretrained Livnium quantum embeddings (`quantum_embed/text_encoder_quantum.py`), dimension fixed by the checkpoint.
  - `ecw_bt`: Loads rotated/pairwise-refined ECW-BT embeddings from `checkpoints/vectors_pairwise.npy` (default) with vocab from `../ecw-BT/data/mass_table.json`; mean-pool sentence encoder (`text/ecw_bt_encoder.py`).
...
- Use it when you want better geometry out of the box or to match the reference configs; quantum is now the sole SNLI encoder. ECW-BT encoder is available as an alternative embedding table (no physics changes) if you want to experiment with the rotated/pairwise-refined ECW-BT space.
