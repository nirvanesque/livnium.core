...
def main():
    ...
    if args.rebuild_mass or not mass_path.exists():
        print(f"[mass] building {mass_path} (min_freq={args.min_freq}) from shards...")
        data_loader.build_mass_table(
            ecw_cfg.wiki_shards,
    ...
    # Progress bar total: prefer a cheap estimate based on the mass-table frequencies.
    # This gives tqdm a real total/ETA without doing an extra full scan.
    if p_keep is not None:
        total_tokens = int((p_keep.astype(np.float64) * freqs.astype(np.float64)).sum())
    else:
