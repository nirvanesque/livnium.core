#!/usr/bin/env python3
"""
Shadow Rule 30 Model (Phase 4)

A model that operates entirely in PCA space, using a non-linear decoder
to map geometry back to binary bits.
...
class ShadowRule30Phase4:
    """
    Shadow Rule 30 with non-linear decoder: A dynamics model operating entirely in PCA space.
    """
    
    def __init__(self, pca_model, dynamics_model, decoder_model, tracker, n_components=8, target_energy=None):
        """
        Initialize Shadow Rule 30 with decoder.
    ...
    def step(self, y_t: np.ndarray) -> np.ndarray:
        """
        Apply one step of dynamics: y_{t+1} = F(y_t) + noise
    ...
    def reconstruct_center_column(self, trajectory_pca: np.ndarray) -> np.ndarray:
        ...
        **CRUCIAL CHANGE FROM PHASE 3:**
        Instead of linear approximation (pc1 * correlation), we use the decoder
        to predict binary bits (0 or 1) from PCA coordinates.
        ...
        # decoder.predict_proba returns probabilities, we threshold at 0.5
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Shadow Rule 30 Phase 4: Simulate dynamics with non-linear decoder"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='results',
        help='Directory containing decoder model (default: results)'
    )
    ...
    # This is the mean L2 norm of training PCA states
    with open(data_dir / 'data_splits.pkl', 'rb') as f:
        splits = pickle.load(f)
    
    train_pca = splits['train']['pca'][:, :n_components]
    # Compute mean energy (L2 norm) of training states (vectorized)
    target_energy = float(np.mean(np.linalg.norm(train_pca, axis=1)))
    
    if args.verbose:
        print(f"\nEnergy Injection:")
        print(f"  Target energy (mean training norm): {target_energy:.6f}")
        print(f"  This prevents dynamics from collapsing to zero")
    
    # Load or create tracker for reconstruction
    tracker = ChaosTracker15D(verbose=False)
    
    # Create Shadow Rule 30 with decoder and energy injection
    shadow = ShadowRule30Phase4(
        pca_model, dynamics_model, decoder_model, tracker, 
        n_components=n_components, target_energy=target_energy
    )
    
    # Fit residuals for stochastic driver (learn noise from training data)
    if args.verbose:
    ...
    if args.verbose:
        print(f"\nShadow Rule 30 (Phase 4) Statistics:")
